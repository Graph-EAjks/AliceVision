...
struct TemporalConstraintFunctor
{
    explicit TemporalConstraintFunctor(const double positionWeight, const double orientationWeight,
                ...
                const double c0oWeight, const double c1oWeight, const double c2oWeight, const int firstViews)
    : _firstViews(firstViews)
    {
    ...
    inline static ceres::CostFunction* createCostFunction(const double positionWeight, const double orientationWeight,
                                        const double c0pWeight, const double c1pWeight, const double c2pWeight,
                                        const double c0oWeight, const double c1oWeight, const double c2oWeight, const int firstViews = 0)
    {
        if (firstViews == 2)
        {
...
struct TemporalRegularizationFunctor
{
    explicit TemporalRegularizationFunctor(const double regularizationWeight, const double trackSquareLength, const int paraSize)
    : _regWeight(regularizationWeight),
    _squareLength(trackSquareLength),
    _paraSize(paraSize)
    {
    }
    ...
    bool operator()(const T* const * para, T* residuals) const
    {
        residuals[0] = _regWeight * (sqrt( (para[1][3] - para[0][3]) * (para[1][3] - para[0][3])
                                         ...
                                         + (para[1][5] - para[0][5]) * (para[1][5] - para[0][5]) )- _squareLength);
    ...
    inline static ceres::CostFunction* createCostFunction(const double regularizationWeight,
                                        const double trackSquareLength, const int paraSize)
    {
        auto costFunction =  new ceres::DynamicAutoDiffCostFunction<TemporalRegularizationFunctor>
                    (new TemporalRegularizationFunctor(regularizationWeight, trackSquareLength, paraSize));
...
const double meanFocalLength(const sfmData::SfMData& sfmData)
{
    ...
    {
        const double focalLengthPixels = intrinsic->getParameters().at(0);
        const double focalLength = focalLengthPixels * intrinsic->sensorWidth() / double(intrinsic->w());
...
const double cameraTrackSquareLength(const std::vector<double*> poses)
{
